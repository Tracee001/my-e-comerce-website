...
import { registerCustomer, loginCustomer } from "../utils/auth";
...
const CustomerRegister = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    try {
      const customer = registerCustomer(form);
      loginCustomer(customer.email, customer.password);
    ...
    <section className="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-10">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8 md:p-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Create your customer account to view orders and purchased products.</p>
        </div>

        <form className="space-y-5" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
            <input id="name" name="name" value={form.name} onChange={handleChange} required className="w-full px-4 py-2 border rounded-md text-gray-900 border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
          </div>
          ...
          <div>
            ...
            <input id="phone" name="phone" value={form.phone} onChange={handleChange} className="w-full px-4 py-2 border rounded-md text-gray-900 border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
          </div>

          <div>
            <label htmlFor="address" className="block text-sm font-medium text-gray-700 mb-1">Address</label>
            <input id="address" name="address" value={form.address} onChange={handleChange} className="w-full px-4 py-2 border rounded-md text-gray-900 border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
          </div>
        ...
        <p className="text-center text-gray-600 mt-6 text-sm">
          Already have an account? <a href="/customer/login" className="text-purple-600 hover:underline font-medium">Sign In</a>
        </p>
      </div>
    </section>
  );
